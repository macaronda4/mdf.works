...
koma/index.html             https://mdf.works/koma/                Koma 概要
koma/tool.html              https://mdf.works/koma/tool.html       連番PNG → APNG・WebP 変換ツール本体
...
`kuroko/tool.html` と `koma/tool.html` はツール本体で、どちらも CSS と JavaScript を内包した1ファイルです。
外部ライブラリや CDN への依存はありません。`koma/tool.html` の画面まわりの CSS は
`kuroko/tool.html` の `<style>` をそのまま土台にしています（片方を直したらもう片方も確認してください）。
APNG とアニメーションWebP のファイル構造は、このページの JavaScript が自前で組み立てています。

`*/tool.html` は全画面レイアウトなので、`chrome.py` は CSS 埋め込みとテーマ切替ボタンの対象外にしています。
