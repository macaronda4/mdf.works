...
POSTS = [
    dict(slug='webfont', cat='開発', date='2026-09-26',
         title='文字が一瞬あとから出る — ウェブフォントをやめた話',
         name='文字が一瞬あとから出る — ウェブフォントをやめた話',
         lead='読み込み直後に文字が消えている、あるいは別の書体で出てから入れ替わる。'
              'どちらも読み込み方の設定です。選べる4つの振る舞い、日本語で容量が桁違いになる理由、'
              '分割して配る方法とその代償、代替書体の字幅を合わせて揺れを抑える手法。'
              'そしてこのサイトが使うのをやめた判断まで。',
         tags=['フォント', 'CSS', '表示速度', '日本語', '個人開発'],
         icon='<path d="M4.6 19.4L10.6 4.6h2.8l6 14.8"/>'
              '<path d="M7.4 14.4h9.2"/>'),
    dict(slug='backup', cat='基本', date='2026-09-23',
         title='スマホのバックアップに入らないもの — 復元してから気づく前に',
         name='スマホのバックアップに入らないもの — 復元してから気づく前に',
         lead='バックアップを取っていたのに、認証アプリのコードだけ消えていた。この種の抜けは、'
              '復元した瞬間まで気づけません。何が入って何が入らないのか、同期とバックアップの違い、'
              '暗号化の有無で中身が変わる話、機種変更の手順と、復元後に見る場所を、'
              '失うと痛い順に並べました。',
         tags=['スマートフォン', 'バックアップ', '機種変更', 'iPhone', 'Android'],
         icon='<path d="M12 3.6v9.8"/>'
              '<path d="M8.4 10l3.6 3.4 3.6-3.4"/>'
              '<path d="M4.2 15.4v3.2a1.8 1.8 0 0 0 1.8 1.8h12a1.8 1.8 0 0 0 1.8-1.8v-3.2"/>'),
    dict(slug='ssd', cat='ガジェット', date='2026-09-22',
         title='SSDは容量だけで選ばない — カタログの速度が出ない理由',
         name='SSDは容量だけで選ばない — カタログの速度が出ない理由',
         lead='表示された速度が出るのは最初の数十秒だけ、ということがあります。速い領域を使い切ったあとの落ち込み、'
              '容量が寿命と速度の両方に効く理由、体感を決めているのは連続の速さではないという話、'
              'そして熱で自分から遅くなる仕組みまで。買う前に見る数字を4つに絞りました。',
         tags=['SSD', 'ストレージ', '自作PC', 'ガジェット', '選び方'],
         icon='<rect x="2.8" y="6.2" width="18.4" height="11.6" rx="2"/>'
              '<path d="M6.4 10.2h5.2M6.4 13.6h3.4"/>'
              '<circle cx="17.4" cy="12" r="1.6"/>'),
    dict(slug='cleanup', cat='ブラウザ', date='2026-09-21',
         title='タブを閉じても残るもの — ブラウザに溜まる記録を整理する',
         name='タブを閉じても残るもの — ブラウザに溜まる記録を整理する',
         lead='履歴を消したのに、開いたらログインしたままだった。閲覧履歴とログイン状態と保存データは、'
              '別々の引き出しにあるからです。それぞれ何が入っていて、消すと何が起きるのか。'
              '掃除より先にやるべき設定、プライベートウィンドウが守る範囲、端末を手放すときの手順まで。',
         tags=['ブラウザ', 'Cookie', 'キャッシュ', 'プライバシー', '設定'],
         icon='<path d="M4 6.4h16"/>'
              '<path d="M9.4 6.4V4.6h5.2v1.8"/>'
              '<path d="M6.2 6.4l1 13.2h9.6l1-13.2"/>'
              '<path d="M10.2 10v6M13.8 10v6"/>'),
    dict(slug='usbc', cat='ガジェット', date='2026-09-18',
         title='USB-Cケーブルは見た目で選べない — 端子が同じでも中身が違う',
         name='USB-Cケーブルは見た目で選べない — 端子が同じでも中身が違う',
         lead='同じ形なのに、充電が遅い。映像が出ない。転送が異様に遅い。原因はたいていケーブルです。'
              '端子の内側で何が違うのか、電圧を切り替える仕組み、識別用の部品の役割、'
              '映像と転送が帯域を分け合う話、そして手持ちを仕分けて買い替える基準まで整理しました。',
         tags=['USB', 'ケーブル', '充電', 'ガジェット', '選び方'],
         icon='<path d="M8.4 3.6h7.2v3.4H8.4z"/>'
              '<path d="M12 7v9.4"/>'
              '<circle cx="12" cy="19" r="2.4"/>'
              '<path d="M6.2 12.6h11.6"/>'),
    dict(slug='manager', cat='セキュリティ', date='2026-09-17',
         ...
         icon='<rect x="3.4" y="9.8" width="17.2" height="10.8" rx="2.2"/>'
              ...
              '<path d="M12 14v3"/>'),
    dict(slug='framerate', cat='ゲーム', date='2026-09-02',
         title='144Hzで何が変わるのか — 遅延はどこで積まれているか',
         name='144Hzで何が変わるのか — 遅延はどこで積まれているか',
         lead='モニタを144Hzにしても、思ったほど変わらないことがあります。画面が書き換わる間隔と、'
              '操作が画面に出るまでの時間は別物だからです。マウスを動かしてから光が目に届くまでを段階ごとに分解し、'
              '平均fpsでは見えない引っかかりの正体、パネルの応答と残像対策の副作用、'
              'そして効く順番を整理しました。',
         tags=['ゲーム', 'モニタ', 'リフレッシュレート', '遅延', '自作PC'],
         icon='<rect x="2.6" y="4.4" width="18.8" height="12.6" rx="2"/>'
              '<path d="M8.6 20.4h6.8M12 17v3.4"/>'
              '<path d="M6.4 13.4l2.6-4 2.4 2.6 2.2-3.4 2.8 4.8"/>'),
